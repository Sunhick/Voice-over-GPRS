...
public class Virtualclass extends MIDlet implements CommandListener{
    ...
    public Virtualclass(){
        ...
        t = new Form("Virtual class room");
        ipaddr = new TextField ("IP Address:","25",100, TextField.ANY);
        t.append (ipaddr);
        ...
        record = new Command("Teacher", Command.SCREEN, 2);
        play = new Command("Student", Command.SCREEN, 3);
	...
	if (c == exitCommand) {
		...
		/*
		  ...
		  t.append("Recorder"+ex.getMessage().toString());
            ...
            t.removeCommand(record);
            t.removeCommand(play);
		if(Rflag==true){
            ...
            try
		{
		    ...
		    while(true)
			{
			    if(F)
				{
				    ...
				    rc.startRecord();
				    p.start();
				    Thread.currentThread().sleep(600); // record for 20 milliseconds
				    p.stop();
				    rc.stopRecord();
				    rc.commit();
				    int len = output.size();
